...
// GetLatest retrieves the most recent metric record
func (r *metricsRepository) GetLatest() (*models.Metric, error) {
	var metric models.Metric
	if err := r.db.Order("timestamp DESC").First(&metric).Error; err != nil {
		if err == gorm.ErrRecordNotFound {
...
// GetAverageUsage calculates average resource usage over a duration
func (r *metricsRepository) GetAverageUsage(duration time.Duration) (*models.AverageMetrics, error) {
	since := time.Now().Add(-duration)
	
	var result models.AverageMetrics
...
// GetUsageTrends returns usage trends for a hostname over time
...
// GetSystemStatus returns current system status for all hosts
