...
// MockSystemCollector is a mock implementation of SystemCollector
...
func (m *MockSystemCollector) GetTopProcesses(limit int, sortBy string) ([]models.ProcessInfo, error) {
	args := m.Called(limit, sortBy)
...
func (m *MockSystemCollector) Reset() error {
	args := m.Called()
	return args.Error(0)
}
...
func setupTestRouter() (*gin.Engine, *MockMetricsRepository, *MockSystemCollector) {
	...
	// Create a simple mock that won't panic
	mockCollector := &MockSystemCollector{}
...
func TestGetCurrentMetrics_Success(t *testing.T) {
	t.Skip("Temporarily skipping due to interface refactoring")

	router, mockRepo, _ := setupTestRouter() // mockCollector not used in this test
